...
exports.handler = (event, context, callback) => {
    ...
    //Configuration - Start
    var srcBucket = 'detect-face-cmpe266';
    var managerPhoneNumber = "+16692259177";
    var table = "customer-responses";
    //Configuration - End
    ...
    var params = {
        ...
        Attributes: ["ALL"] // to get the response.
    };
                ...
                docClient.put(paramsDB, function(err, rs) {
                    if (err) {
                        ...
                        if (result.FaceDetails !== null && result.FaceDetails.length > 0) {
                            ...
                            // if atleast it is 80% confidence then it is human face.
                            ...
                            if (item !== null && item.Confidence >= 80) {
                                if (item.Emotions !== null && item.Emotions.length > 0) {
                                    ...
                                    if (pe.Confidence > 80 && (pe.Type.toLowerCase() == 'angry' || pe.Type.toLowerCase()  == 'sad' ||
                                        pe.Type.toLowerCase()  == 'confused' || pe.Type.toLowerCase()  == 'disgusted') && item.AgeRange.Low > 15) {
                                        ...
                                        if (item.AgeRange.High <= 15) {
                                            ...
                                            if (item.Gender.Confidence > 80) {
                                                if (item.Gender.Value == 'Female') {
                                                    ...
                                                    //inside female
                                                    if(item.AgeRange.Low>=20)
                                                    {
                                                        callback(null, 'lipstick.jpg');
                                                    }else if(item.AgeRange.High<20){
                                                        // girl range <20
                                                        // return barbiedoll
                                                        callback(null, 'barbiedoll.jpg');
                                                    }
                                                    else
                                                    {
                                                        //for remaining ages hairdryer
                                                        callback(null, 'hairdryer.jpg');
                                                    }
                                                    return;
